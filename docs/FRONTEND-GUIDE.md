...
Todas las rutas requieren: `Authorization: Bearer <token>` y rol `JUGADOR`.

### 8.1 Perfil del Jugador
...
Response 200:
{ "status": "success", "mensaje": "Perfil actualizado correctamente." }
```

- `telefono`: 9 dígitos exactos, opcional

### 8.3 Crear Reserva

```
POST /api/jugador/reservas
Content-Type: application/json
...
**Reglas:**
- El slot debe estar `DISPONIBLE` u `OFERTA`
- La cancha debe estar `DISPONIBLE`
- El precio se calcula automáticamente según `Tipo_Precio` del horario (BASE/PRIME/BAJA)
- Si el slot está en `OFERTA`, se usa el precio de oferta
- El slot se bloquea automáticamente a `RESERVADO`
- Se aplica comisión del 5%

**Estados de reserva tras crear:**
| Estado | Significado |
|--------|-------------|
| `PENDIENTE` | Creada, esperando pago |

### 8.4 Listar Mis Reservas
...
- Ordenado por `Fecha_Crea` descendente
- `ID_Pago` y `EstadoPago` son `null` si no se ha pagado aún

### 8.5 Detalle de una Reserva
...
```
POST /api/jugador/reservas/:idReserva/pagar
Authorization: Bearer <token>
...
- Se crea el registro de pago con estado `PAGADO`
- La reserva pasa a `CONFIRMADA`
- Se emite notificación en tiempo real al dueño vía Socket.io

### 8.7 Cancelar Reserva
...
{
  "status": "success",
  "mensaje": "Reserva cancelada. Reembolso del 50% será procesado.",
  "porcenReemb": 50
}
```

**Política de cancelación según tiempo restante:**

| Antelación | Zona | Reembolso |
|------------|------|:---------:|
| ≥ 48 horas | `TEMPRANA` | 100% |
| 24–48 horas | `INTERMEDIA` | 50% |
| < 24 horas | `TARDIA` | 0% |
| Sin pago previo | `SIN_PAGO` | 0% |

- El slot se libera automáticamente a `DISPONIBLE`
- Si hay reembolso, se crea un registro en la tabla `Reembolso`
- Si la reserva ya está `CANCELADA` o `NO_SHOW`, se rechaza
...
### 8.9 Crear Review de una Cancha

```
POST /api/jugador/canchas/:idCancha/reviews
Content-Type: application/json
...
**Reglas:**
- `calificacion`: entero entre 1 y 5 (obligatorio)
- `comentarios`: máximo 200 caracteres (opcional)
- Solo se puede dejar **una** review por cancha por usuario

### 8.10 Listar Mis Reviews

```
GET /api/jugador/reviews
Authorization: Bearer <token>
...
## 9. WebSocket en Tiempo Real (Socket.io)
...
### 9.2 Eventos que emite el backend
...
| `reserva-cancelada` | `{ idReserva, idCancha, jugadorNombre, porcenReemb, zonaCancela }` | Un jugador cancela una reserva confirmada |
...
| `Reservas` | ✅ CRUD vía jugador + dueño lecturas |
| `Pagos` | ✅ Creación vía jugador + lecturas |
| `Oferta` | ✅ Creación vía dueño |
| `Suscripcion` | ✅ Lectura (saldo + historial liquidaciones) |
| `Liquidacion` | ✅ Lectura (dashboard + historial) |
| `Reembolso` | ✅ Creación vía cancelación de reserva |
| `Comprobantes` | ❌ Sin endpoints |
| `Reviews` | ✅ CRUD vía jugador |
| `Tokens_Recup` | ❌ Sin endpoints |
...
MÓDULO JUGADOR (token + rol JUGADOR)
  ── Perfil ──
  GET /api/jugador/perfil
  PUT /api/jugador/perfil
...
_Generado el 15/06/2026 — Backend v1.0 (Momentos 1, 2, 3 + Jugador completos)_
